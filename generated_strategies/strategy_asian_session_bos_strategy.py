...
def asian_session_bos_strategy(df: pd.DataFrame, params: Optional[Dict] = None) -> pd.Series:
    """
    ASIAN SESSION BOS STRATEGY
    ...
    Exit:
      - Journal every trade (setup screenshot, entry/exit, RR achieved) to build your own accuracy statistics instead of trusting someone else's.
    
    Stop Loss:
      - Stop loss: typically placed beyond the swept high/low (not explicitly stated in the reel, but standard practice for this kind of setup — plan your own stop placement and position size before entering).
      - Define your stop-loss rule precisely (e.g., a few pips beyond the sweep wick) so it's consistent, not discretionary.
    
    Take Profit:
      - Risk-Reward (RR): minimum 1:2 to 1:3 per trade.
      - After the sweep, wait for a Break of Structure (BOS) in the opposite direction. 7 Enter in the direction of the BOS. 8 Target a minimum 1:2–1:3 risk-reward ratio.
...
if __name__ == "__main__":
    ...
    # signals = asian_session_bos_strategy(df)
    pass
