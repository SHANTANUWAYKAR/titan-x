"""
EMA Crossover Trading Strategy (1)
...
def ema_crossover_trading_strategy_1(df: pd.DataFrame, params: Optional[Dict] = None) -> pd.Series:
    """
    EMA Crossover Trading Strategy (1)
    
    Timeframe: unspecified
    Instruments: any
    Required Indicators: EMA_21, EMA_9
    
    Extracted Rules:
    
    Exit:
      - 📊 What is EMA? EMA (Exponential Moving Average) is a technical indicator that tracks the moving price of the market while giving more importance to recent data. It helps traders: •Identify trends •Find entry and exit points •Reduce market noise
    
    Stop Loss:
      - ⚙ Strategy Setup To use this strategy, you need to apply two EMAs on your chart: •9 EMA → Fast Moving Average •21 EMA → Slow Moving Average
      - 💡 Why This Strategy Works •Simple and beginner-friendly •Based on trend-following principles •Helps avoid emotional trading •Works across multiple timeframes ⚠ Important Tips:- •Always use stop-loss to manage risk •Avoid trading in sideways markets •Combine with volume or support/resistance for better accuracy
    ...
    required_indicators = ['EMA_21', 'EMA_9']
    ...
    # Using EMA_21 and EMA_9 from pre-computed indicators
    
    try:
        # Long entry: Fast EMA crosses above Slow EMA
        # CLARIFICATION: "crossover" = candle close confirms cross
        long_cross = (df['EMA_21'] > df['EMA_9']) & (df['EMA_21'].shift(1) <= df['EMA_9'].shift(1))
        
        # Short entry: Fast EMA crosses below Slow EMA
        short_cross = (df['EMA_21'] < df['EMA_9']) & (df['EMA_21'].shift(1) >= df['EMA_9'].shift(1))
...
if __name__ == "__main__":
    ...
    # signals = ema_crossover_trading_strategy_1(df)
    pass
