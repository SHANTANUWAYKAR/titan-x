...
def liquidity_sweep_strategy(df: pd.DataFrame, params: Optional[Dict] = None) -> pd.Series:
    """
    LIQUIDITY SWEEP STRATEGY
    ...
    Stop Loss:
      - Stop Loss|||Above the liquidity sweep candle high||
      - Stop Loss||Below the sweep candle low||
    
    Take Profit:
      - Target|Risk Reward Previous Day Low strong rejection wick If the next candle closes inside the previous candle range, plan a|1 : 2|Previous Day Low. should appear.|Buy Entry|
...
if __name__ == "__main__":
    ...
    # signals = liquidity_sweep_strategy(df)
    pass
