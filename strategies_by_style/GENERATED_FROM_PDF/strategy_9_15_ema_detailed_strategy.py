...
def strategy_9_15_ema_detailed_strategy(df: pd.DataFrame, params: Optional[Dict] = None) -> pd.Series:
    """
    9 15 EMA DETAILED STRATEGY
    
    Timeframe: 1m, 3m, 5m, 1d
    Instruments: any
    Required Indicators: EMA_15, EMA_9
    
    Extracted Rules:
    
    Entry (Long):
      - EMA crosses above 15 EMA
      - EMA should move upward strongly
      - EMA should form approximately 30° angle
      - Wait for first retest
      - Enter after bullish rejection candle
    
    Entry (Short):
      - EMA crosses below 15 EMA
      - EMA should move downward strongly
      - Wait for first retest
      - Enter after bearish rejection candle
      - Place stop loss above swing high
    
    Stop Loss:
      - Place stop loss above swing high
      - Slight slope = Weak momentum
    ...
    required_indicators = ['EMA_15', 'EMA_9']
    ...
    # Using EMA_15 and EMA_9 from pre-computed indicators
    
    try:
        # Long entry: Fast EMA crosses above Slow EMA
        # CLARIFICATION: "crossover" = candle close confirms cross
        long_cross = (df['EMA_15'] > df['EMA_9']) & (df['EMA_15'].shift(1) <= df['EMA_9'].shift(1))
        
        # Short entry: Fast EMA crosses below Slow EMA
        short_cross = (df['EMA_15'] < df['EMA_9']) & (df['EMA_15'].shift(1) >= df['EMA_9'].shift(1))
...
if __name__ == "__main__":
    ...
    # signals = 9_15_ema_detailed_strategy(df)
    pass
