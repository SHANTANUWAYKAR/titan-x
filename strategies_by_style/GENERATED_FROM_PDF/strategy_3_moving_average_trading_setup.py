"""
3 MOVING AVERAGE TRADING SETUP
...
def strategy_3_moving_average_trading_setup(df: pd.DataFrame, params: Optional[Dict] = None) -> pd.Series:
    """
    3 MOVING AVERAGE TRADING SETUP
    
    Timeframe: 1d
    Instruments: any
    Required Indicators: EMA_20, EMA_3, EMA_50, EMA_9
    
    Extracted Rules:
    
    Exit:
      - Exit Rules:
      - Exit the trade when an opposite crossover occurs (9 EMA crosses below 20 EMA).
    ...
    required_indicators = ['EMA_20', 'EMA_3', 'EMA_50', 'EMA_9']
    ...
    # Using EMA_20 and EMA_3 from pre-computed indicators
    
    try:
        # Long entry: Fast EMA crosses above Slow EMA
        # CLARIFICATION: "crossover" = candle close confirms cross
        long_cross = (df['EMA_20'] > df['EMA_3']) & (df['EMA_20'].shift(1) <= df['EMA_3'].shift(1))
        
        # Short entry: Fast EMA crosses below Slow EMA
        short_cross = (df['EMA_20'] < df['EMA_3']) & (df['EMA_20'].shift(1) >= df['EMA_3'].shift(1))
...
if __name__ == "__main__":
    ...
    # signals = 3_moving_average_trading_setup(df)
    pass
